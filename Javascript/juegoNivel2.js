...
function playerMove(moveName, damage, tipoMovimiento) {
    ...
    if (opponent.hp <= 0) {
        ...
        if (currentOpponentIndex >= opponentTeam.length) {
            logMessage("¡Campeon de tipo Roca se ha quedado sin Pokemon! ¡Has ganado el primer combate!");
            detenerTemporizador();
            win = 1;
            if (win == 1) {
            ...
            logMessage(`¡${opponent.name} ha sido debilitado! ¡Campeón de tipo Roca saca a ${opponentTeam[currentOpponentIndex].name}!`);
